...
def extract_publication(entry):
    ...
    if entry_type is None:
        ...
        editor = None if editor is None else entry.get(editor, None)
        editor = process_special_character(editor)
        title = is_valid_key(entry_parsed_keys, "title".upper())
        title = None if title is None else entry.get(title, None)
        title = process_special_character(title)
        publisher = is_valid_key(entry_parsed_keys, "publisher".upper())
        publisher = None if publisher is None else entry.get(publisher, None)
        ...
        series = None if series is None else entry.get(series, None)
        series = process_special_character(series)
        address = is_valid_key(entry_parsed_keys, "address".upper())
        address = None if address is None else entry.get(address, None)
        edition = is_valid_key(entry_parsed_keys, "edition".upper())
        edition = None if edition is None else entry.get(edition, None)
        month = is_valid_key(entry_parsed_keys, "month".upper())
        month = None if month is None else entry.get(month, None)
        note = is_valid_key(entry_parsed_keys, "note".upper())
        note = None if note is None else entry.get(note, None)
        ...
        chapter = None if chapter is None else entry.get(chapter, None)
        pages = is_valid_key(entry_parsed_keys, "pages".upper())
        pages = None if pages is None else entry.get(pages, None)
        publisher = is_valid_key(entry_parsed_keys, "publisher".upper())
        publisher = None if publisher is None else entry.get(publisher, None)
        ...
        book_type = None if book_type is None else entry.get(book_type, None)
        address = is_valid_key(entry_parsed_keys, "address".upper())
        address = None if address is None else entry.get(address, None)
        edition = is_valid_key(entry_parsed_keys, "edition".upper())
        edition = None if edition is None else entry.get(edition, None)
        ...
        book_title = None if book_title is None else entry.get(book_title, None)
        ...
        organization = None if organization is None else entry.get(organization, None)
        ...
        how_published = None if how_published is None else entry.get(how_published, None)
        year = is_valid_key(entry_parsed_keys, "year".upper())
        year = None if year is None else entry.get(year, None)
        month = is_valid_key(entry_parsed_keys, "month".upper())
        month = None if month is None else entry.get(month, None)
        ...
        school = None if school is None else entry.get(school, None)
        year = is_valid_key(entry_parsed_keys, "year".upper())
        year = None if year is None else entry.get(year, None)
        month = is_valid_key(entry_parsed_keys, "month".upper())
        month = None if month is None else entry.get(month, None)
        ...
        keywords = None if keywords is None else entry.get(keywords, None)
        ...
        type = None if type is None else entry.get(type, None)
        ...
        institution = None if institution is None else entry.get(institution, None)
        ...
        report_type = None if report_type is None else entry.get(report_type, None)
