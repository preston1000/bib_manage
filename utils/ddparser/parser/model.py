...
def epoch_evaluate(args, model, loader, punctuation):
    ...
    for batch_index, inputs in enumerate(loader(), start=1):
        ...
        loss = loss_function(connection_prob, deprel_prob, connections, deprel, mask)
        connection_predict, deprel_predict = decode(args, connection_prob, deprel_prob, mask)
        # ignore all punctuation if not specified
        if not args.punct:
            punct_mask = layers.reduce_all(
                layers.expand(layers.unsqueeze(words, -1), (1, 1, punctuation.shape[0])) !=
                layers.expand(layers.reshape(punctuation, (1, 1, -1)), words.shape + [1]),
                dim=-1)
...
def epoch_predict(env, args, model, loader):
    ...
    for batch, inputs in enumerate(loader(), start=1):
        ...
        connection_predicts, deprel_predicts = decode(args, connection_prob, deprel_prob, mask)
        connections.extend(layers.split(nn.masked_select(connection_predicts, mask), lens.numpy().tolist()))
        deprels.extend(layers.split(nn.masked_select(deprel_predicts, mask), lens.numpy().tolist()))
        if args.prob:
            arc_probs = nn.index_sample(layers.softmax(connection_prob, -1), layers.unsqueeze(connection_predicts, -1))
    ...
    probabilities = [[round(p, 3) for p in seq.numpy().tolist()] for seq in probabilities]
