...
class ArgConfig(configparser.ConfigParser):
    ...
    def __init__(self, args=None):
        ...
        model_g.add_arg(
            "--model_files",
            default="model_files/ddparser_models",
            help="Directory path to save model and ",
        )

        data_g = ArgumentGroup(parser, "ddparser_data", "Data paths, vocab paths and ddparser_data processing options")
        data_g.add_arg("--train_data_path", help="path to training ddparser_data.")
        data_g.add_arg("--valid_data_path", help="path to valid ddparser_data.")
        data_g.add_arg("--test_data_path", help="path to testing ddparser_data.")
        ...
        run_type_g.add_arg(
            ...
            "-p",
            action="store_true",
            help="whether to preprocess the ddparser_data first",
        )
        run_type_g.add_arg(
            "--use_data_parallel",
            action="store_true",
            help="The flag indicating whether to use ddparser_data parallel mode to train the model.",
        )
        run_type_g.add_arg(
            "--seed",
            "-s",
...
class Environment(object):
    ...
    def __init__(self, args):
        ...
        if not os.path.exists(args.fields_path) or args.preprocess:
            logging.info("Preprocess the ddparser_data")
