...
def evaluate(env):
    ...
    start = datetime.datetime.now()
    loss, metric = epoch_evaluate(arguments, model, dataset.loader, punctuation)
...
def predict(env):
    ...
    dataset = TextDataset(predicts, [env.WORD, env.FEAT], arguments.buckets)  # 只需提取word和feat
    # set the ddparser_data loader
    dataset.loader = batchify(dataset, arguments.batch_size)
    ...
    start = datetime.datetime.now()
    model.eval()
    connection_predicts, deprel_predicts, predict_prob = epoch_predict(env, arguments, model, dataset.loader)
    total_time = datetime.datetime.now() - start
    ...
    logging.info("Save the predicted result to {}".format(arguments.infer_result_path))
